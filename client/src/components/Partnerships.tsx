...
const partners = [
  {
    ...
    logo: microsoft,
    description:
      "Empowering every person and organization on the planet to achieve more through innovative software and cloud solutions.",
  },
  {
    name: "Lenovo",
    logo: lenovo,
    description:
      "Delivering smarter technology for all with cutting-edge devices and IT solutions.",
  },
  {
    name: "Dell",
    logo: dell,
    description:
      "Empowering businesses and individuals with innovative technology solutions to drive growth and productivity.",
  },
  {
    name: "Hewlett-Packard",
    logo: HP,
    description:
      "Revolutionizing the way people work and live with sustainable and innovative technology solutions.",
  },
];
          ...
          {partners.map(({ name, logo, description }) => (
            <Card key={name} className="shadow-lg hover:scale-105 transition-transform">
              <CardHeader className="flex flex-col items-center space-y-4 p-6">
                <img src={logo} alt={`${name} logo`} className="w-24 h-24 object-contain block" />
                <CardTitle className="text-center text-lg font-semibold">{name}</CardTitle>
              </CardHeader>
